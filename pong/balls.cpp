...
void Balls::update(const Ship &ship, float deltaTime) {
  for (auto &ball : m_balls) {

    ball.m_translation += ball.m_velocity * deltaTime;
    // Wrap-around

    if (ball.m_translation.y < -0.95f)
      ball.m_velocity.y = -1*(ball.m_velocity.y);
    if (ball.m_translation.y > +0.95f)
      ball.m_velocity.y = -1*(ball.m_velocity.y);
  }
}
...
Balls::Ball Balls::makeBall(glm::vec2 translation,
  ...
  positions.emplace_back(glm::vec2{-0.1f, +0.1f});
  positions.emplace_back(glm::vec2{+0.1f, +0.1f});
  positions.emplace_back(glm::vec2{+0.1f, -0.1f});
  positions.emplace_back(glm::vec2{-0.1f, -0.1f});
